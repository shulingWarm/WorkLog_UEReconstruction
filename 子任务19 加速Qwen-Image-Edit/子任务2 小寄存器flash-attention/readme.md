...
- [DONE] 解决GPU上Q\*K^T结果错误的问题。
	- [DONE] 验证GPU上第一个周期的读取数据内容上GPU和CPU是否一致。
		- [DONE] 打印GPU上第一个周期读取的内容。
		- [DONE] 打印CPU上第一个周期读取的内容。
		- [DONE] 验证发现CPU和GPU的打印内容，第一个周期相同，第二个周期不一致。
		- [DONE] 研究CPU和GPU的结果不一致的原因。
